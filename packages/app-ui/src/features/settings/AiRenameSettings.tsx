...
export function AiRenameSettings() {
  ...
  const enabled = (renameSettingsQ.data?.['aiRename.enabled'] as boolean) ?? true;
  const modelId = (renameSettingsQ.data?.['aiRename.modelId'] as string) ?? null;
            ...
            <div className="border-t pt-4">
              ...
              {models.length === 0 ? (
                <div className="text-muted-foreground border-border/40 flex flex-col items-center gap-2 rounded-lg border border-dashed py-6 text-xs">
                  ...
                  {t('aiRename.noModel', {
                    defaultValue: '暂无可用模型，请先在「模型供应商」中配置 Provider 并启用模型。',
                  })}
                </div>
              ) : (
