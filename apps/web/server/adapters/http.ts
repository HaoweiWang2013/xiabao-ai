/**
 * HttpPort 实现：Node 18+ 原生 fetch
 */
import type { FetchInit, FetchResponse, HttpPort } from '@xiabao/core';

export function createWebHttpAdapter(): HttpPort {
  return {
...
function asyncIterableToReadable(iter: AsyncIterable<Uint8Array>): ReadableStream<Uint8Array> {
  ...
  return new ReadableStream<Uint8Array>({
    async pull(controller) {
      ...
      if (r.done === true) controller.close();
      else controller.enqueue(r.value);
    },
    async cancel(reason) {
      if (typeof it.return === 'function') await it.return(reason);
    },
  });
}
